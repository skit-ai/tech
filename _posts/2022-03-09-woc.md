...
Text-To-Speech (TTS) systems of Skit.ai, as well as TTS systems in general, have a tendency to mix some ambient noise along with the speech it outputs. This aim of this research project was to remove that noise and quantify how well the noise has been removed using standard metrics.
...
We apply our methods on 2 different datasets: First on the public [NOIZEUS](https://ecs.utdallas.edu/loizou/speech/noizeus/) dataset and next on a dataset created by the in-house TTS systems of Skit.ai. The results are quite satisfactory when we apply our methods on the NOIZEUS dataset and we found that the Wiener Filter and the Kalman Filters perform the best outperforming one another for different signal-to-noise ratios (SNR).
